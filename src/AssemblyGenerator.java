...
public class AssemblyGenerator {
        ...
        if(params == null){
            ...
            for(int i = 0; i < params.size(); i++){
                if(params.get(i).getType() instanceof ArrayType){
                    ...
                    Type next = params.get(i).getType();
                    String tempString = "";
                    do {
                        tempString += "$";
                        next = ((PointerType) next).next();
                    } while (next instanceof PointerType);
                    temp += "." + next.getName() + tempString;
                }
                else {
    ...
    public void writeLocalInitWithConst(STO sto, STO expr)
    {
        ...
        if(sto.getLoad() || (sto instanceof VarSTO && (((VarSTO)sto).getPbr() || ((VarSTO)sto).getisSet() || ((VarSTO) sto).getInsideStruct() != null)))
        {
            ld(o1, o1);
        }
    ...
    public void writeLocalInitWithVar(STO sto, STO expr, STO temp, boolean flag)
    {
        ...
        if(sto.getLoad() || (sto instanceof VarSTO && (((VarSTO)sto).getPbr() || flag || ((VarSTO)sto).getisSet())))
        {
            ld(o1,o1);
        }
    ...
    */
    ...
    public STO writeIntegerBinaryArithmeticExpression(STO a, BinaryOp o, STO b, STO result)
    {
        ...
        if(a.isConst() && !a.getIsAddressable())
        {
            ...
            if(a.getLoad() || (a instanceof VarSTO && (((VarSTO)a).getPbr() || ((VarSTO)a).getisSet() )))
            {
                ld(l7, l7);
            }
            writeAssembly(AssemlyString.LD + "\t\t\t" + AssemlyString.LOAD + "\n", "%l7", "%o0");
        ...
        if(b.isConst() && !b.getIsAddressable())
        {
            ...
            if(b.getLoad() || (b instanceof VarSTO && (((VarSTO)b).getPbr() || ((VarSTO)b).getisSet() )))
            {
                ld(l7, l7);
            }
            writeAssembly(AssemlyString.LD + "\t\t\t" + AssemlyString.LOAD + "\n", "%l7", "%o1");
    ...
    public void writeArrayIndexCheck(STO des, STO expr, Type array, STO sto)
    {
        if(array.isArray()) {
            ...
            if(des.getLoad() || (des instanceof VarSTO && (((VarSTO)des).getPbr() || ((VarSTO)des).getisSet() )))
            {
                ld(o0,o0);
            }
    ...
    public void writePointerComparison(STO a, BinaryOp o , STO b , STO result)
    {
        ...
        else
        {
            ...
            if(a.getLoad() || (a instanceof VarSTO && (((VarSTO)a).getisSet() || ((VarSTO)a).getPbr() )))
            {
                ld(l7, l7);
            }
        ...
        else
        {
            ...
            if(b.getLoad() || (b instanceof VarSTO && (((VarSTO)b).getisSet() || ((VarSTO)b).getPbr() )))
            {
                ld(l7, l7);
            }
    ...
    public int writeReturn(STO expr, FuncSTO callingFunc, int offset){
        ...
        if(expr != null){
            ...
            //ADDED BY DAISY - > CORRECTNESS GK TERJAMIN HAHA
                ...
                if(args.get(i) instanceof VarSTO){
                    if(((VarSTO) param.get(i)).getPbr()){
                        ...
                        if(!(args.get(i).getType() instanceof ArrayType)){
                            ...
                            if(args.get(i).getLoad() || (args.get(i) instanceof VarSTO && (((VarSTO)args.get(i)).getPbr() || ((VarSTO)args.get(i)).getisSet())))
                            {
                                ld(l7, l7);
                            }
    ...
    int writeArrayStructDecl(STO sto, Vector<STO> parameters, int offset, boolean global, Type t){
        String structName;
        structName = t.getName() + "." + t.getName();
    ...
    int writeArrow(STO left, STO newLeft, STO right, int offset){
        ...
        if(left.getLoad() || (left instanceof VarSTO && (((VarSTO)left).getisSet() || ((VarSTO)left).getPbr())))
        {
            ld(l7, l7);
        }
    ...
    void writeGlobalStack(){
        while(!structStack.empty()){
            String global = structStack.pop();
            String useless = structStack.pop();
